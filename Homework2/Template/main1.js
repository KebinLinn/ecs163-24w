...
d3.csv("pokemon.csv").then(rawData => {
  ...
  rawData.forEach(function (d) {
    d.speed = Number(d.Attack);
    d.weight = Number(d.Defense);
    d.name = (d.Name)
  });
  rawData = rawData.filter(d => !isNaN(d.weight) && d.weight != "" && d.isLegendary === "True");
  ...
  g1.append("text")
    ...
    .text("Pokemon Attack and Defense Scatterplot");
  // X label
  g1.append("text")
    .attr("x", scatterWidth / 2)
  ...
  const legend = svg.append("g")
    .attr("class", "legend")
    .attr("transform", `translate(${scatterWidth + scatterMargin.left + 10}, ${scatterMargin.top})`);

  const legendRectSize = 13;
  const legendSpacing = 4;
  const columnWidth = 100;
  const legends = legend.selectAll(".legend")
    .data(rawData) // Assuming you want to use rawData here
    .enter()
    .append("g")
    .attr("class", "legend")
    .attr("font-size", "12")
  ...
  rawData.forEach(function (d) {
    d.name = d.Name;
    d.total = +d.Total; // Convert to number
});

rawData = rawData.filter(d => d.isLegendary === "True");

// Now you don't need the reduce step, as we are not counting occurrences anymore
r = rawData.map(({ name, total }) => ({ name, total }));
...
const titleText = "Pokemon Total Stats";
